...
function rodash_add(augend, addend)
    value = 0
    if rodash_isNumber(augend) then
        value += augend
    end if
    if rodash_isNumber(addend) then
        value += addend
    end if
    return value
end function
...
function rodash_asDateString(format = "long-date" as String, asLocal = false as Boolean) as String
    return rodash_internal_getDateObject(asLocal).asDateString(format)
end function
...
function rodash_asSeconds(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).asSeconds()
end function
...
function rodash_clone(value = Invalid as Dynamic) as Dynamic
    ...
    else if rodash_isString(value) then
        clonedValue = "" + value
    else if rodash_isNumber(value) then
        clonedValue = 0 + value
    else if rodash_isBoolean(value) then
...
function rodash_compact(array as Object) as Object
    ...
    if (type(array) = "roArray" AND NOT array.isEmpty()) then
        for each item in array
            if (type(item) = "<uninitialized>" OR item = Invalid) OR (rodash_isString(item) AND item.isEmpty()) then
                continue for
            else if rodash_isNumber(item) then
                if item = 0 then
                    continue for
                end if
            else if rodash_isBoolean(item) then
                if item = false then
                    continue for
                end if
            end if
...
function rodash_concat(array as Object, values as Object) as Object
    ...
    returnArray = rodash_clone(array)
...
function rodash_createNode(nodeType = "Node" as String, fields = {} as Dynamic) as Object
    node = createObject("roSGNode", nodeType)
    if rodash_isNode(node, nodeType) then
        fieldsType = type(fields)
        if (fieldsType = "roAssociativeArray" AND fields.keys().count() > 0) OR (fieldsType = "roArray" AND NOT fields.isEmpty()) then
...
sub rodash_debounce(callback as Function, wait = 0 as Dynamic, options = {
    ...
    if NOT m.doesExist(id) then
        ...
        timer.observeFieldScoped("fire", (sub(event as Object)
            ...
            if NOT m.doesExist(localId) then
                return
            end if
...
function rodash_differenceBy(array = CreateObject("roArray", 0, true) as Object, values = CreateObject("roArray", 0, true) as Object, iteratee = Invalid as Dynamic) as Object
    iterateeIsFunction = rodash_isFunction(iteratee)
    iterateeIsProperty = NOT iterateeIsFunction AND rodash_isString(iteratee)
...
function rodash_dropWhile(array = CreateObject("roArray", 0, true) as Object, predicate = Invalid)
    ...
    for i = 0 to array.count() - 1
        ...
        else if rodash_isString(predicate) then
...
' // => [{ "user": "fred", "active": false, "age": 40 }]
function rodash_filter(sourceArray as Object, predicate as Dynamic) as Object
    ...
    if (type(sourceArray) = "roArray" AND NOT sourceArray.isEmpty()) then
        ...
        for each item in sourceArray
            ...
            if rodash_isFunction(predicate) then
                ...
                ' Add item to filteredArray if it matches the predicate
                if rodash_isBoolean(item[predicate]) then
...
function rodash_findIndex(array, predicate = Invalid as Dynamic, fromIndex = 0 as Integer, strict = false as Boolean) as Integer
    for index = fromIndex to array.count() - 1
        ...
        else if (type(item) = "roAssociativeArray") AND rodash_isString(predicate) AND item.doesExist(predicate) then
            if item[predicate] then
                return index
            end if
        end if
...
function rodash_fromISO8601String(dateString = "" as String, asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).fromISO8601String(dateString)
end function
...
' @param {Boolean} asLocal - Whether to get the local date. Default is false.
' @returns {Object} The date object.
function rodash_fromSeconds(numSeconds = 0 as Integer, asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).fromSeconds(numSeconds)
end function
...
function rodash_get(aa as Object, keyPath as String, fallback = Invalid as Dynamic, validator = rodash_isNotInvalid as Function) as Dynamic
    ...
    for each key in keyPath.split(".")
        ...
        else if (rodash_isNotInvalid(nextValue) AND getInterface(nextValue, "ifArrayGet") <> Invalid) then
            index = val(key, 10)
...
function rodash_getDayOfMonth(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getDayOfMonth()
end function
...
function rodash_getDayOfWeek(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getDayOfWeek()
end function
...
function rodash_getHours(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getHours()
end function
...
function rodash_getLastDayOfMonth(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getLastDayOfMonth()
end function
...
function rodash_getMilliseconds(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getMilliseconds()
end function
...
function rodash_getMinutes(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getMinutes()
end function
...
function rodash_getMonth(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getMonth()
end function
...
function rodash_getNodeChildren(node as Object) as Object
    if rodash_isNode(node) then
...
function rodash_getNodeLastChild(node as Object) as Object
    children = rodash_getNodeChildren(node)
...
function rodash_getSeconds(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getSeconds()
end function
...
function rodash_getYear(asLocal = false as Boolean) as Integer
    return rodash_internal_getDateObject(asLocal).getYear()
end function
...
function rodash_groupBy(collection as Object, iteratee = Invalid as Dynamic) as Object
    ...
    else if rodash_isString(iteratee) then
        for each value in collection
...
' @ignore
function rodash_internal_getDateObject(asLocal = false as Boolean) as Object
...
function rodash_isArrayLike(value as Dynamic) as Boolean
    return (type(value) <> "<uninitialized>" AND value <> Invalid) AND getInterface(value, "ifArrayGet") <> Invalid
end function
...
function rodash_isEmpty(value as Dynamic)
    if (type(value) = "<uninitialized>" OR value = Invalid) OR rodash_isNumber(value) OR rodash_isBoolean(value) then
        return true
    else
...
function rodash_isEmptyString(value as Dynamic) as Boolean
    return rodash_isString(value) AND value.isEmpty()
end function
...
function rodash_isFinite(value as dynamic) as boolean
    if NOT rodash_isNumber(value) then
        return false
    end if
...
function rodash_isLength(value as Dynamic) as Boolean
    if rodash_isInteger(value) AND rodash_isFinite(value) AND (value >= 0) then
        return true
    end if
    return false
end function
...
' rodash.isList({}) // => false
' rodash.isList("") // => false
function rodash_isList(value as Dynamic) as Boolean
    return type(value) = "roList"
end function
...
function rodash_isNaN(value as Dynamic) as Boolean
    return NOT rodash_isNumber(value)
end function
...
function rodash_isNumber(value as Dynamic) as Boolean
    valueType = type(value)
    return valueType = "Integer" OR valueType = "roInt" OR valueType = "roInteger" OR valueType = "LongInteger" OR valueType = "Float" OR valueType = "roFloat" OR valueType = "Double" OR valueType = "roDouble" OR valueType = "roIntrinsicDouble"
end function
...
function rodash_map(collection = CreateObject("roAssociativeArray") as Dynamic, iteratee = Invalid as Dynamic)
    ...
    if rodash_isString(collection) then
    ...
    for each item in collectionToProcess
        if rodash_isString(iteratee) then
...
function rodash_maxBy(array = CreateObject("roArray", 0, true) as Object, iteratee = Invalid) as Dynamic
    ...
    else if rodash_isString(iteratee) AND (type(array[0]) = "roAssociativeArray") then
        maxValue = array[0]
        for each value in array
...
function rodash_merge(target as Object, sources as Object) as Object
        ...
        for each key in source
            if target.doesExist(key) then
                ...
                if type(target[key]) = "roAssociativeArray" AND type(source[key]) = "roAssociativeArray" then
                    ...
                    target[key] = rodash_merge(target[key], source[key])
                else
...
function rodash_multiply(multiplier as dynamic, multiplicand as dynamic) as dynamic
    if (NOT rodash_isNumber(multiplier)) OR (NOT rodash_isNumber(multiplicand))
        return 0
    end if
...
sub rodash_removeNode(nodeOrEvent as Dynamic)
...
sub rodash_removeNodeChildren(node as Object)
    if rodash_isNode(node) then
...
function rodash_size(collection = Invalid as Dynamic)
    if (type(collection) = "<uninitialized>" OR collection = Invalid) OR (NOT (type(collection) = "roArray") AND NOT (type(collection) = "roAssociativeArray") AND NOT rodash_isString(collection)) then
        return []
    end if
    if (type(collection) = "roAssociativeArray") then
        collection = rodash_toArray(collection)
    else if rodash_isString(collection) then
...
function rodash_slice(array = CreateObject("roArray", 0, true) as Object, startPos = 0 as Integer, endPos = -1 as Integer) as Object
    if NOT (type(array) = "roArray") then
        return Invalid
    end if
    if endPos = -1 then
    ...
    return rodash_clone(array).slice(startPos, endPos)
end function
...
function rodash_sortBy(collection = Invalid as Dynamic, iteratee = Invalid as Dynamic)
    ...
    if (type(iteratee) = "roArray") then
        for each iteration in iteratee
            if rodash_isString(iteration) then
...
function rodash_subtract(minuend as Dynamic, subtrahend as Dynamic) as Dynamic
    if (NOT rodash_isNumber(minuend)) OR (NOT rodash_isNumber(subtrahend))
        return 0
    end if
...
function rodash_times(n = 0 as Integer, iteratee = Invalid as Dynamic) as Object
    returnArray = CreateObject("roArray", 0, true)
    if NOT rodash_isInteger(n) then
        return returnArray
    end if
    for i = 0 to n - 1
