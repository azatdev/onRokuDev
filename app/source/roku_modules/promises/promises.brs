...
function promises_onThen(promise as dynamic, callback = invalid as dynamic, context = "__INVALID__" as object) as dynamic
    if callback = invalid then
        callback = promises_internal_defaultThenCallback
    end if
...
function promises_onCatch(promise as dynamic, callback = invalid as dynamic, context = "__INVALID__" as object) as dynamic
    if callback = invalid then
        callback = promises_internal_defaultCatchCallback
    end if
...
function promises_onFinally(promise as dynamic, callback = invalid as dynamic, context = "__INVALID__" as object) as dynamic
    if callback = invalid then
        callback = promises_internal_defaultFinallyCallback
    end if
...
function promises_isPromiseEvent(event as dynamic) as boolean
...
sub promises_setMessagePort(port as dynamic)
    m.__promises__promisesPort = port
end sub

function promises_getMessagePort() as dynamic
...
function promises_getMessage(port as dynamic) as dynamic
    if promises_peekMessage(port) <> invalid then
...
function promises_wait2(timeoutMilliseconds as dynamic, port as dynamic) as dynamic
    promisesPort = promises_getMessagePort()
    ...
    while true
        ...
        if portIsPromisesPort then
            ...
            if promises_isPromiseEvent(event) then
            ...
            messagePort = promises_getMessagePort()
            if messagePort <> invalid then
                promises_peekMessage(messagePort)
            end if
...
sub promises_internal_clearPromiseStorage(promise as object, nodeEvent = invalid as dynamic)
...
function promises_internal_getPromiseStorage(promise as object, nodeEvent = invalid as dynamic) as object
    if nodeEvent <> invalid then
        id = "__promises__" + nodeEvent.getNode()
    else
        id = "__promises__" + promise.id
    ...
    if storage = invalid then
        if m.__promises__promisesPort <> invalid then
            ...
            callback = sub(event as dynamic)
                ...
                promises_internal_delay(sub(context as dynamic)
        ...
        promises_internal_observeFieldScoped(promise, "promiseState", callback, [
            "promiseResult"
        ])
        storage = {
            promise: promise
...
sub promises_internal_notifyListeners(event as object)
    ...
    if not promises_isComplete(originalPromise) then
        return
    end if
    ...
    while promises_internal_hasStorage(originalPromise)
        ...
        promiseStorage = promises_internal_getPromiseStorage(originalPromise, event)
        ...
        promises_internal_clearPromiseStorage(originalPromise, event)
        ...
        for each listener in promiseStorage.thenListeners
            promises_internal_processPromiseListener(originalPromise, promiseState, listener, promiseState = "resolved", true, promiseResult)
        end for
        'handle .catch() listeners
        for each listener in promiseStorage.catchListeners
            promises_internal_processPromiseListener(originalPromise, promiseState, listener, promiseState = "rejected", true, promiseResult)
        end for
        'handle .finally() listeners
        for each listener in promiseStorage.finallyListeners
            promises_internal_processPromiseListener(originalPromise, promiseState, listener, true, false, promiseResult)
        end for
...
sub promises_internal_processPromiseListener(originalPromise as object, originalPromiseState as string, storageItem as object, callCallback as boolean, isThenOrCatch as boolean, promiseValue = invalid as dynamic)
        ...
        if promises_isPromise(callbackResult) then
            ...
            promises_onThen(callbackPromise, sub(result as dynamic, context as dynamic)
                if context.originalPromiseState = "resolved" then
                    promises_resolve(context.originalPromiseResult, context.newPromise)
                else
                    promises_reject(context.originalPromiseResult, context.newPromise)
                end if
            end sub, context)
            ...
            if originalPromiseState = "resolved" then
                promises_resolve(promiseValue, newPromise)
            else
                promises_reject(promiseValue, newPromise)
            end if
        end if
    end if
end sub

function promises_internal_defaultThenCallback(value = invalid as dynamic, _ = invalid as dynamic) as dynamic
    return value
end function

function promises_internal_defaultCatchCallback(value = invalid as dynamic, _ = invalid as dynamic) as dynamic
    return promises_reject(value)
end function

sub promises_internal_defaultFinallyCallback(_ = invalid as dynamic)
end sub
...
function promises_internal_observeFieldScoped(node as object, field as string, callback as dynamic, infoFields = [] as object) as boolean
...
function promises_internal_unobserveFieldScoped(node as object, field as string) as boolean
    if not type(node) = "roSGNode" then
        return false
    else
