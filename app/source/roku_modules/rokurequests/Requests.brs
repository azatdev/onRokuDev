...
function rokurequests_Requests() as Object
    return {
        request : rokurequests_Requests_request,
        get: rokurequests_Requests_getRequest,
        post: rokurequests_Requests_postRequest
    }
end function

function rokurequests_Requests_getRequest(url as String, args=invalid)
    return m.request("GET", url, args)
end function

function rokurequests_Requests_postRequest(url as String, args=invalid)
    return m.request("POST", url, args)
end function

function rokurequests_Requests_request(method, url as String, args as Object)
    ...
    rc = rokurequests_Requests_cache(method, url, headers)
    ...
    if response = invalid
        response = rokurequests_Requests_run(method, url, headers, data, _timeout, _retryCount, _verify, _parseJson, _parseJsonFlags)
...
function rokurequests_Requests_run(method, url, headers, data, timeout, retryCount, verify, parseJson, parseJsonFlags)
    ...
    return rokurequests_Requests_response(urlTransfer, responseEvent, requestDetails)

end function



function rokurequests_Requests_cache(method as String, url as String, headers as Object)
...
function rokurequests_Requests_headers()
...
function rokurequests_Requests_queryString()

    return {
        _urlTransfer: CreateObject("roUrlTransfer"),
        _qs_array: [],
        addString: sub(params as String)
                if rokurequests_Requests_Utils_inString("&", params)
                    split_params = params.split("&")
                    for each param in split_params
                        if rokurequests_Requests_Utils_inString("=", param)
                ...
                else if rokurequests_Requests_Utils_inString("=", params)
                    split_params = params.split("=")
        ...
        append: function(url as String) as String
            ' "append the QS on a provided URL"
                if m._qs_array.Count() > 0
                    if rokurequests_Requests_Utils_inString("?", url)
                        if url.right(1) = "?"
                            return url + m.build()
                        else
                            return url + "&" + m.build()
...
function rokurequests_Requests_response(urlTransfer as Object, responseEvent as Object, requestDetails as Object)
...
function rokurequests_RequestsUrlTransfer(enableEncodings as Boolean, retainBodyOnError as Boolean, verify as String)
...
function rokurequests_Requests_Utils_inString(char as String, strValue as String)
